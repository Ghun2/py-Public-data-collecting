...
def add_column(res,bjdcd):
    in_sgg = str(bjdcd[:5])
    in_bjd = str(bjdcd[5:])

    platcd = str(res['pnu'][10])

    bunji = res['mnnmSlno']
    if len(bunji.split('-')) == 2:
        bunji = bunji.split('-')
        bun = bunji[0]
        ji = bunji[1]
    elif len(bunji.split('-')) == 1:
        bun = bunji
        ji = ''
    else :
        bun = ''
...
def mdb_insert_one(res,bjdcd):
    ...
    sql = 'INSERT INTO getPossessionAttr (%s) VALUES (%s);' % (keystr,var_string)
    # print(sql)
...
def rest_call(BJD_code):
    ...
    url = 'http://apis.data.go.kr/1611000/nsdi/PossessionService/attr/getPossessionAttr?'

    params = 'pnu=' + BJD_code + '&numOfRows=0&ServiceKey='+ curr_key
    ...
    if reqnum >= 40000:
        ...
        for i in range(1,divcount+1):
            params = 'pnu=' + BJD_code + '&pageNo='+ str(i) +'&numOfRows='+ str(numrow) +'&ServiceKey='+ curr_key
        ...
        params = 'pnu=' + BJD_code + '&numOfRows=200000&ServiceKey='+ curr_key
...
for j in range(3295,4000):   #index 287 divide 7 이제 8하면됨
    plc = totplc[j]
    BJD_code = str(plc[0])
